...
public static class Extensions
{
    ...
    {
        return new Vector3((float)Math.Round((decimal)vector.X, 2), (float)Math.Round((decimal)vector.Y, 2),
            (float)Math.Round((decimal)vector.Z, 2));
    }

    public static Point Round(this Point point, int scale)
    {
        return new Point((int)Math.Floor(point.X / (float)scale) * scale,
    ...
    {
        return new Vector2((int)(vector.X / gridSize) * gridSize - gridSize,
            (int)(vector.Y / gridSize) * gridSize - gridSize);
    }

    public static Vector3 TranslateToGrid(this Vector3 vector, float gridSize = 1)
    {
        return new Vector3((int)(vector.X / gridSize) * gridSize - gridSize,
