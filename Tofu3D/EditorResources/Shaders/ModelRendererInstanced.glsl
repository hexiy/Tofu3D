...
out vec4 albedoBoundingBox;
out vec2 albedoTiling;
out vec2 albedoOffset;
void main(void)
{
	...
	albedoTiling = u_tiling;
	albedoOffset = u_offset;
	//	float tiling = 2;
	uv = a_uv;
	//	if(uv.x < 0.5){
	//		uv.x = uv.x - 2;
	...
	// Apply tiling and offset to local UVs (repeating behavior handled with `fract`)
	//	vec2 tiledUV = fract(a_uv * u_tiling + u_offset);

	// Map tiled UVs to the atlas' sub-region
	//	uv = albedoBoundingBoxInAtlasStart + tiledUV * albedoBoundingBoxSize;
...
in vec4 albedoBoundingBox;
in vec2 albedoTiling;
in vec2 albedoOffset;
in vec2 uvOffset;
out vec4 fragColor;
...
void main() {
	...
	vec2 boundingBoxStart = albedoBoundingBox.xy;
	vec2 boundingBoxEnd = albedoBoundingBox.zw;
	vec2 boundingBoxSize = boundingBoxEnd - boundingBoxStart;


	vec2 uvCoords = uv * albedoTiling + albedoOffset + (uvOffset);
