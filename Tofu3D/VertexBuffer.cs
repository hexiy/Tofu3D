...
{
	int ElementsPerVertex { get; init; }
	// public int Vbo { get; private init; }
	private int SizeOfElementInBytes { get; init; }
	VertexAttribPointerType VertexAttribPointerType { get; init; }

	private VertexBuffer(int elementsPerVertex /*, int vbo*/, int sizeOfElementInBytes, VertexAttribPointerType vertexAttribPointerType)
	{
	...
	public static VertexBuffer Create<T>(BufferTarget bufferTarget, T[] vertexData, int elementsPerVertex)
	{
		...
		{
			throw new ArgumentNullException("Define VertexBuffer data value type(float,uint) as the generic parameter Create<T>");
		}
		...
		if (bufferTarget == BufferTarget.ArrayBuffer)
		{
			...
			GL.BufferData(bufferTarget, sizeOfElementInBytes * vertexDataFloats.Length, vertexDataFloats, BufferUsageHint.StaticDraw);
		}
		...
		if (bufferTarget == BufferTarget.ElementArrayBuffer)
		{
			...
			GL.BufferData(bufferTarget, sizeOfElementInBytes * vertexDataInts.Length, vertexDataInts, BufferUsageHint.StaticDraw);
		}

		VertexBuffer vertexBuffer = new VertexBuffer(elementsPerVertex, sizeOfElementInBytes, vertexAttribPointerType);
		return vertexBuffer;
	}
	...
	public void EnableAttribs(bool sequential = true, params int[] countsOfElements)
	{
		...
		{
			GL.VertexAttribPointer(nextAttribIndex, countOfElements, VertexAttribPointerType, false, ElementsPerVertex * SizeOfElementInBytes, (IntPtr) (sequential ? (currentAttribOffset * SizeOfElementInBytes) : 0));
