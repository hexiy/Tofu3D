...
public class TweenManager
{
    ...
    public void Update()
    {
        for (int i = ActiveTweens.Count - 1; i >= 0; i--)
        {
            ...
            if (ActiveTweens[i].CurrentTime >= 0) ActiveTweens[i].OnUpdate?.Invoke(ActiveTweens[i].GetValue());

            if (isCompleted)
            {
                if (ActiveTweens[i].GetLoop() == Tween.LoopType.Restart) ActiveTweens[i].CurrentTime = 0;
