...
public class SpriteSheetBatcher : Batcher
{
    public SpriteSheetBatcher(int size, Material material, Texture texture) : base(size, material, texture)
    {
        vertexAttribSize = 10;
    }
    ...
    public override void Render()
    {
        ...
        // ATTRIB: drawOffset -   2 floats
        GL.VertexArrayAttribBinding(vao, 5, 1);
        GL.EnableVertexArrayAttrib(vao, 5);
        GL.VertexArrayAttribFormat(
                                   vao,
                                   5, // attribute index, from the shader location = 4
                                   2, // size of attribute
                                   VertexAttribType.Float, // uint representation of a color
                                   false,
                                   32); // relative offset, first item


        GL.VertexArrayVertexBuffer(vao, 1, vbo_attribs, IntPtr.Zero, sizeof(float) * 10);
